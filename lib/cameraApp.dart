...
Future<Welcome> getVal(
    String k) async //to get the description model of the image the user wants to mimick
    {
...
class _CameraAppState extends State<CameraApp> {
  ...
  String tempStore;
  Future capturePicture(context) async {
    try {
      final path = join(
  ...
  void handleTime(Timer timer) async
  {
    if (time != 0) {
      setState(() {
        time--;
      ...
      String response = await capturePicture(context);
